...
`yt-dlp --flat-playlist` returns titles in YouTube's default locale, while oEmbed returns the original-language title that bookmarking stores. `Playmark.Channel.enrich_titles/1` re-fetches each flat title via oEmbed in parallel (bounded concurrency + timeout) so the video list matches what a bookmark would save; a failed lookup falls back to the flat title. Resolved titles are memoized in `Playmark.Cache` under a `{:title, id}` key (a video's title is effectively immutable), so reopening a channel or repeating a search only issues oEmbed requests for ids not already cached. A failed lookup is not cached.
