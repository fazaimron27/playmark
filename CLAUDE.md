...
- **Contexts** (`Playmark.Bookmarks`, `Playmark.Subscriptions`, `Playmark.Playlists`) — Ecto-backed CRUD over `Bookmark` / `Subscription` / `Playlist` schemas. `Bookmarks.add_bookmark/1` and `Subscriptions.add_subscription/1` first run the URL through `Playmark.YouTube.validate/1` (see below), so a blank field or non-YouTube paste fails instantly instead of after a slow oEmbed / `yt-dlp` round-trip in the `:fetching` state. `add_subscription/1` also canonicalizes the URL first (`YouTube.canonical_channel_url/1`) and resolves the channel name through the `:channel_impl` seam (so it's stubbable in tests without shelling out).
- **`Playmark.YouTube`** — deliberately permissive URL validation: confirms a string is an `http(s)` URL on a known YouTube host (`youtube.com` and subdomains, `youtu.be`), not that it points at a real video/channel — only YouTube can say that. Being stricter risks rejecting legitimate link shapes, which is worse than the slow failure it replaces. Also holds `canonical_channel_url/1`, which strips a trailing channel-tab segment (`/videos`, `/streams`, `/shorts`, …) so a subscription is stored as one canonical bare URL and the app chooses which tab to fetch — `.../@handle/videos` and `.../@handle` normalize to the same subscription.
- **External-IO modules** (`Playmark.Metadata`, `Playmark.Channel`, `Playmark.Search`, `Playmark.Local`, `Playmark.Playback` + its `Playmark.Player.Mpv`/`Playmark.Player.Vlc` backends) — the oEmbed / `yt-dlp` / filesystem / player boundary. Each shells out, hits the network, or reads the disk and returns a tagged tuple (`{:ok, _}` / `{:error, reason}`). `Playmark.Search` runs a `yt-dlp` `ytsearchN:` query and reuses `Playmark.Channel`'s `parse_videos/1` + `enrich_titles/1`, so a search result is shaped identically to a channel video (including its `:live` tag) and flows into the same `:videos` mode — the two callers' `--print` templates must stay in lockstep with the shared parser. `Playmark.Local.list_files/1` reads a directory's top-level media files and returns the same `%{id, title, url}` shape (here `url` is the file path), so local files flow into `:videos` mode too.
...
`/` opens an incremental filter over the current browse list, applied in **every** browse list — the four top-level views (bookmarks, subscriptions, local playlists) and the shared `:videos` list (channel videos / search results / local files). The queue and history modals are out of scope (bounded, curated, own key sets). In the **search** view's `:list` mode `/` stays the query prompt (that view has no list there); everywhere else `/` enters `:filter`.

The whole thing is a pure in-memory substring match, **not** ripgrep: these lists are dozens of maps already in TUI state, so filtering is a microsecond `String.contains?` — shelling out would need a subprocess + spawned Task and wouldn't narrow live as you type. `Playmark.TUI.Filter` is the single source of truth for *which list + which fields + apply the term*, mirroring `Actions.current_list/1`; `Actions.current_list/1` and `View.body/1` both resolve through `Filter.visible/1`, so what's navigated and what's rendered can never diverge. Match fields per view: bookmarks `[:title, :channel]`, subscriptions `[:name, :url]`, playlists `[:name, :path]`, videos `[:title]` (case-insensitive). Rows may be Ecto structs or plain video maps — both answer `Map.get/2`, so `matches?/3` treats them uniformly.

The term lives in `state.filter` (a plain string the handlers edit — no `TextInput` NIF, so it stays unit-testable) with `state.filter_return` remembering the base mode (`:list`/`:videos`) to restore. While the field is open, `state.mode` is `:filter`, so `Filter` resolves the *effective* mode through `filter_return` (a filter opened over `:videos` still narrows the video list, not the underlying view's list). In `:filter` mode: printable keys append, `backspace` deletes, and after every term change `selected` is reclamped into the narrowed list; `Enter`/`Esc` close the field **keeping** the term (empty term = no filter). In the base mode, `Esc` is the clear key — clears the filter when one is active, else its existing behavior (a no-op in `:list`; back-to-list in `:videos`). Because navigation/play/delete/queue all resolve through `current_list/1`, they act on the *filtered* selection. The filter resets to `""` on `Tab` (view switch), leaving `:videos`, and each fresh `:videos`/`:search`/`:files` result.

### Subscriptions and local playlists are never stored with their contents

A subscription persists only the channel URL and name. The video list is fetched live via `Playmark.Channel.list_videos/2` (using `yt-dlp --flat-playlist`) each time a channel is opened, so it's always current. Don't add a videos column or cache. The stored URL is canonicalized on add (`YouTube.canonical_channel_url/1` strips a trailing tab segment), so a channel is held as one bare URL and the app picks the tab. `list_videos/2`'s second arg is `tab` (`:videos` | `:streams`), appended to that bare URL — so the same subscription fetches either the uploads or the live/streams tab deterministically. In a channel's video list the `s`/`v` keys flip between the two tabs in place (re-fetching via `Actions.switch_tab/2`, which is a no-op unless `channel_url` is set — i.e. only on a subscription listing, not on search results or local files); a failed tab switch keeps the current list on screen rather than dropping back to `:list`. The Streams tab badges each row's live status in a Status column.

Each video map carries a `:live` tag — `:live` / `:ended` / `:upcoming` / `:none` — parsed from yt-dlp's `%(live_status)s` field by `Channel.parse_videos/1`. That parser is shared with `Playmark.Search`, so the two callers' `--print` templates and the parser must stay in lockstep; the parser tolerates a legacy 2-field line by defaulting `:live` to `:none`, and `enrich_titles/1` preserves `:live` (it only overwrites `:title`/`:author`). Live playback needs no special handling — a live URL flows through the normal play path and joins at the live edge (no `--live-from-start`).
