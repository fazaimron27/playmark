...
- A static `:help` overlay (`?` from any browse mode, excluding `:help` itself so a second `?` closes it) mirrors the Queue/History open/close/return shape: `Actions.open_help/1` sets `mode: :help` + `help_return: state.mode`, and `handle_help_key/2` restores `help_return` on `Esc`/`?`. It spawns no task and reads no state — `View.help_body/0`/`help_text/0` is a hand-authored keybinding reference kept in lockstep with the handlers (footers only surface a mode's most relevant keys).
- Every browse-list mover (`move/2`, `move_channel_playlist/2`, `move_queue/2`, `move_history/2`, `move_explore/2`, `move_search/2`) accepts an integer delta *or* a `:top`/`:bottom` jump target resolved by the shared `jump_index/2`. `g`/`Home` → `:top`, `G`/`End` → `:bottom`, `PageUp`/`PageDown` → `±@page_step` (fixed step, default 10 — there is no terminal-height value in TUI state, so paging is a fixed step, not a real screenful; `clamp/3` bounds the ends). Every mode's key handler wires these six codes.
