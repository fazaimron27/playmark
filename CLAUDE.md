...
- **`Playmark.TUI`** — the `ex_ratatui` app shell. Owns the runtime callbacks (`mount/1`, `handle_event/2`, `handle_info/2`, `render/2`) and the **browse-core result handlers** — the `handle_info` clauses that commit a channel-tab, channel-playlist, local-directory, or YouTube-playlist listing, because those write the browse-core keys `Actions` owns. Every *overlay's* result handler lives with that overlay instead: `handle_info` matches the message tag and forwards the whole tuple, so each module below owns both halves of its own message — the spawn and the commit. It delegates to:
  ...
  - **`Playmark.TUI.PlaybackActions`** — `start_play/4` (the single funnel every play routes through, and where history is written), the resume prompt, the `stream_plan/2` / `captions_plan/2` / `play_steps/2` step list, and the two handlers that commit what the backend reports: `handle_progress/2` folds each visual stage into the `playing` map this module built, and `handle_result/2` commits the exit, advancing or halting the queue.
  ...
  - **`Playmark.TUI.SearchActions`** / **`ExploreActions`** — the two transient overlays. Both open over any browse mode and terminate their task on cancel; Search additionally owns a query sub-machine and its own filter term. Each owns its own `handle_result/2`, including the stale-ref clause that drops a canceled or superseded page.
  - **`Playmark.TUI.AddActions`** — the two ways a row enters the DB: `start_add/2` (from the input field, dispatching on `state.view`) and `bookmark_video/2` (from a video row), each with its commit half — `handle_result/2` and `handle_bookmark_result/2`.
  - **`Playmark.TUI.HelpActions`** — the static `:help` overlay.
  - **`Playmark.TUI.Status`** — pure text builders for the footer: `count_with_label/3` / `count_with_number/2` (a pluralized tally) and `add_error/2` (why an add failed, mapping a unique-index violation to a per-target "already saved" message). No state, no IO. `count_with_label/3` had drifted into three near-copies that were not identical; shared from here, a new caller cannot fork a fourth.
...
The task always sends to the runtime process — `parent = self()` is captured in `handle_event/2`, so `handle_info/2` is always the receiver. What varies is where the clause that *commits* it lives. Overlay messages (`:search_result`, `:explore_result`, `:add_result`, `:bookmark_video_result`, `:play_progress`, `:play_result`) are matched by one forwarding clause in `tui.ex` and handled in the module that spawned them; browse-core listings are handled in `tui.ex` itself. Forwarding is a plain function call made after delivery, so a new handler must keep the whole message tuple as its first argument — including the stale-ref fallthrough, which belongs with its group rather than duplicated on both sides of the boundary.
