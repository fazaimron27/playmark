...
- A mode state machine (`:list`, `:input`, `:fetching`, `:loading`, `:videos`, `:playing`, `:confirm`) gates which keys do what and which async results are accepted. `:confirm` stages a destructive action (list `d` delete, or queue `c` clear) behind a `y`/`n` prompt shown in the footer — `y` performs it, any other key cancels; the underlying list/queue stays on screen. The pending action and the mode to restore afterward live in `state.confirm` / `state.confirm_return` (see `Actions.handle_confirm_key/2`).
